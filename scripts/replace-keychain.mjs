...
const destDir = 'C:\\Users\\15ind\\OneDrive\\Desktop\\Quellflow\\code\\ResinArt\\artvelle\\public\\products\\gallery\\Personalised Gifts\\Name Keychain';

async function replaceImages() {
  const files = fs.readdirSync(artifactsDir);
  const heroFile = files.find(f => f.startsWith('keychain_hero_') && f.endsWith('.png'));
  const gallery45File = files.find(f => f.startsWith('keychain_gallery_45_') && f.endsWith('.png'));
  const lifestyleFile = files.find(f => f.startsWith('keychain_lifestyle_') && f.endsWith('.png'));
  const closeupFile = files.find(f => f.startsWith('keychain_closeup_') && f.endsWith('.png'));
  ...
  console.log('Successfully replaced images for Name Keychain!');
}
