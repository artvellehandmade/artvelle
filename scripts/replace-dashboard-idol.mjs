...
const destDir = 'C:\\Users\\15ind\\OneDrive\\Desktop\\Quellflow\\code\\ResinArt\\artvelle\\public\\products\\gallery\\Car Accessories\\Dashboard Idol';

async function replaceImages() {
  const files = fs.readdirSync(artifactsDir);
  const heroFile = files.find(f => f.startsWith('dashboard_idol_hero_') && f.endsWith('.png'));
  const gallery45File = files.find(f => f.startsWith('dashboard_idol_gallery_45_') && f.endsWith('.png'));
  const lifestyleFile = files.find(f => f.startsWith('dashboard_idol_lifestyle_') && f.endsWith('.png'));
  const closeupFile = files.find(f => f.startsWith('dashboard_idol_closeup_') && f.endsWith('.png'));
  ...
  console.log('Successfully replaced images for Dashboard Idol!');
}
